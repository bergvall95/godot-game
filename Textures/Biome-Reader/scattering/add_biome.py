...
def search_file_in_env(
    ...
    #benchmark debug
    dprint(f"search_file_in_env({file_name})->start")
...
def import_biome_objs(
    ...
    for obj_names in to_import.values():
        for name in obj_names:
            if (name not in bpy.data.objects):
                warned = name
                print(f"SCATTER5 WARNING: couldn't import '{name}' from {path_found}")
            continue
...
class SCATTER5_OT_load_biome(bpy.types.Operator): 
    ...
    def add_quickaction_instruction(self, action_type, mask_name,):
        ...
        def instruction():
            ...
            if (action_type=="vg"):
                #directly call a vg paint operator after scatter
                bpy.ops.scatter5.vg_quick_paint(mode="vg", group_name=mask_name,)

            elif (action_type=="bitmap"):
                #directly call a image paint operator after scatter
                bpy.ops.scatter5.image_utils(option="paint", paint_color=(1,1,1), img_name=mask_name,)

            elif (action_type=="curve"):
                #directly call a curve operator, but our operator is window/area dependent so we need an override
                    ...
                    for area in screen.areas:
                        if (area.type=='VIEW_3D'):
                            ...
                            if (region is not None):
                                #override context AND use invoke at the same time! third param is use undo, operator adds its own undo state, so this is not needed (i hope?)
    ...
    def add_security_instruction(self, max_count, total_count,):
        ...
        def instruction():
            ...
            if (total_count<-2147483646) or (total_count>2147483646):
                bpy.ops.scatter5.popup_dialog(
                    'INVOKE_DEFAULT',
                    msg=translate("Your terrain is way too big, blender can't scatter over 2 billion 147 millions instances"),
    ...
    def update_progress(self,context):
        ...
        context.scene.scatter5.operators.load_biome.progress_bar = ((self.step+1)/(self.max_step+1))*100
        context.scene.scatter5.operators.load_biome.progress_label = list(self.Operations.keys())[self.step] if (self.step<len(self.Operations.keys())) else translate("Done!")
    ...
    def restore(self, context):
        ...
        context.scene.scatter5.operators.load_biome.progress_bar = 0
        context.scene.scatter5.operators.load_biome.progress_label = ""
        context.scene.scatter5.operators.load_biome.progress_context = ""
        ...
        if ("Pop Message" not in self.Operations):
            ...
            if (context.scene.scatter5.operators.create_operators.f_sec_verts_allow==True):
                if (not context.scene.scatter5.operators.create_operators.f_display_bounding_box) and (not context.scene.scatter5.operators.load_biome.f_display_biome_allow):
                    for p in emitter.scatter5.particle_systems:
                        if (p.name in self.loaded_psys):
                            for o in p.get_instance_objs():
                                if (o.display_type=="BOUNDS"):
