...
def ensure_texture_ptr_name():
    ...
    for sys in all_systems:
        ...
        if (type(sys).__name__=="SCATTER5_PR_particle_systems"):
            
            mod = sys.get_scatter_mod(strict=False, raise_exception=False,)
            if (mod is None):
                print("ensure_texture_ptr_name() SKIP: Couldn't find scatter-engine modifier ", sys.name)
                continue
        ...
        #or in group..
        elif (type(sys).__name__=="SCATTER5_PR_particle_groups"):
            
            members = sys.get_psy_members()
            if (not len(members)):
                print("ensure_texture_ptr_name() SKIP: Couldn't find scatter-engine nodegroup for this Scatter-group: ", sys.name)
                continue
        ...
        for ptrname,ngname in propgroup:
            ...
            if (baseng is None):
                print(f"ERROR: Couldn't find '{ngname}' in scatter-engine for 'particle_systems[{sys.name}].{ptrname}'")
                return None
            
            #get the texture node
            tx = baseng.node_tree.nodes.get("texture")
            if (tx is None):
                print(f"ERROR: Couldn't find 'particle_systems[{sys.name}].{ptrname}.texture'")
                return None
            
            #if no ng assigned, fine, we assign default one (shouldn't happen)
            nt = tx.node_tree
            if (nt is None):
                print(f"ERROR: 'particle_systems[{sys.name}].{ptrname}.texture.node_tree' is None")
                return None
            
            #finally, we update the pointer, we temporarily disable our properties updates
            with bpy.context.scene.scatter5.factory_update_pause(factory=True):
                setattr(sys,ptrname,nt.name)
...
class SCATTER5_OT_scatter_texture_enum(bpy.types.Operator):
    ...
    def generate_items(scene, context, ):
        ...
        items.append(None)
        items.append((".TEXTURE *DEFAULT* MKIV", translate("None"), ""))
